 ...
 * @param {org.acme.model.Reward} reward The sample reward instance.
 * @transaction
 */
 ...
 function Reward(reward){
    return getParticipantRegistry('org.acme.model.User')
        .then(function(participantRegistry){
            if(participantRegistry.exists(reward.currentPost.owner) && participantRegistry.exists(reward.currentParticipant)){
                reward.currentParticipant.reward.push(reward.currentPost);
                reward.currentPost.owner.token -= 5;
                reward.currentParticipant.token += 5;
                return participantRegistry.update(reward.currentParticipant);    
            }
            else return false;
        })
        .then(function(){
        ...
        .then(function(assetRegistry){
            reward.currentPost.rewarders.push(reward.currentParticipant);
            reward.currentPost.value += 5;
            return assetRegistry.updateAll([reward.currentPost.owner ,reward.currentParticipant]);
        })
        .catch(function (error) {
